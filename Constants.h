#define N_ROW 4     // number of rows
#define N_COL 6     // number of columns

#define NORTH 3
#define EAST  2
#define WEST  1
#define SOUTH 0

#define NORTH_WALL 8    // 2^3
#define EAST_WALL  4    // 2^2
#define WEST_WALL  2    // 2^1
#define SOUTH_WALL 1    // 2^0

#define MAP_DIMS 2    // number of dimensions
#define ROW_CODE 0    // index of row information in the array
#define COL_CODE 1    // index of column information in the array

#define TRUE 1
#define FALSE 0

#define N_WALL 4
#define N_SENSORS 3
#define FRONT 0
#define LEFT 1
#define RIGHT 2

#define STACK_SIZE 30

#define MOVE_FORWARD 0
#define MOVE_LEFT 1
#define MOVE_RIGHT 2
#define MOVE_BACKWARD 3

#define QUEUE_SIZE 24
#define MAX_VALUE 10000

#define GAME_QUEUE_SIZE 50
#define GAME_DICT_SIZE 50
#define GAME_TRACK_SIZE 50
#define SOLUTION_MAX_LENGTH 20
